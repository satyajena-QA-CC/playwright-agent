...
// 259, ...) so each checkout attempt — a retry, or a separate concurrent
// load instance — gets a fresh, traceable email. Shares the same counter
// file as tests/recorded.spec.js, so numbering stays consistent whichever
// one you run.
...
// either writes back the increment, handing out a duplicate email (this was
// observed in practice under concurrent load). mkdirSync is atomic on every
// platform (fails with EEXIST if the directory already exists), so it
// doubles as a cheap cross-process mutex here.
...
// Number of concurrent end-to-end checkout runs. Scale this up to 1000 by
// raising LOAD_COUNT — pair it with a matching --workers value so they
// actually run in parallel instead of queuing a few at a time.
const LOAD_COUNT = parseInt(process.env.LOAD_COUNT || '10', 10);

for (let i = 1; i <= LOAD_COUNT; i++) {
  test(`checkout flow (load ${i}/${LOAD_COUNT})`, async ({ page }) => {
